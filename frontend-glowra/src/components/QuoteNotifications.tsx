...
export default function QuoteNotifications() {
  ...
  useEffect(() => {
    ...
    const stored = safeGetItem('quoteNotifications');
    if (stored) {
  ...
  const checkForNewNotifications = async () => {
    try {
      ...
      const lastCheck = safeGetItem('lastNotificationCheck');
      ...
      safeSetItem('lastNotificationCheck', new Date().toISOString());
    } catch (error) {
  ...
  const addNotification = (notification: Notification) => {
    setNotifications((prev) => {
      ...
      safeSetItem('quoteNotifications', JSON.stringify(updated));
  ...
  const clearAll = () => {
    setNotifications([]);
    setUnreadCount(0);
    safeRemoveItem('quoteNotifications');
  };
